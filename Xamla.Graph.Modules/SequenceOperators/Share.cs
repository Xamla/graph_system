...
{
    [Module(ModuleType = "Xamla.Sequence.Operators.Share")]   
    public class Share
        : ModuleBase
    {
        GenericInputPin inputPin;
        GenericInputPin readAheadCountPin;
        GenericOutputPin outputPin;

        GenericDelegate<Func<object, object, object>> genericDelegate;

        public Share(IGraphRuntime runtime)
            : base(runtime)
        {
            this.inputPin = AddInputPin("Input", PinDataTypeFactory.FromType(typeof(ISequence<>)), PropertyMode.Never);
            this.readAheadCountPin = AddInputPin("ReadAheadCount", PinDataTypeFactory.Create<int>(1, WellKnownEditors.IntNumber), PropertyMode.Default);
        ...
        public IInputPin ReadAheadCountPin
        {
            get { return readAheadCountPin; }
        }
        ...
        [EvaluateInternal]
        private ISequence<T> EvaluateInternal<T>(ISequence<T> input, int readAheadCount)
        {
            return input.Share(readAheadCount);
        }
        ...
        protected override Task<object[]> EvaluateInternal(object[] inputs, CancellationToken cancel)
        {
            ...
            var input = inputs[0];
            var readAheadCount = inputs[1];

            var result = genericDelegate.Delegate(input, readAheadCount);
